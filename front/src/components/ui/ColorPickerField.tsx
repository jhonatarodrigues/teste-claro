...
const suggestedColors = ['#E7FF2F', '#8BFF3D', '#22D3EE', '#00B37E', '#F97316', '#EF4444', '#A855F7', '#F472B6'];
...
export function ColorPickerField({ label = 'Cor do time', value, onChange, error }: ColorPickerFieldProps) {
  const [visible, setVisible] = useState(false);
  const [draftValue, setDraftValue] = useState(value);
  const pickerModule = visible ? (require('reanimated-color-picker') as typeof import('reanimated-color-picker')) : null;
  ...
  return (
    <View className="gap-2">
      ...
      <Pressable onPress={openPicker} className="rounded-xl bg-app-input px-4 py-4">
        <View className="flex-row items-center justify-between">
          <View>
            <Text className="text-white">{value.toUpperCase()}</Text>
            <Text className="mt-1 text-xs text-[#7A7A86]">Toque para abrir o seletor de cor</Text>
          </View>
          <View className="h-10 w-10 rounded-full border border-white/10" style={{ backgroundColor: value }} />
        </View>
      </Pressable>
          ...
          <View className="flex-1 justify-end bg-black/60">
            <View className="rounded-t-[28px] bg-app-surface px-5 pb-8 pt-5">
              <View className="items-center">
                ...
                <Text className="mt-4 text-xl font-semibold text-white">Escolha a cor do time</Text>
              ...
              <View className="mt-6 rounded-2xl bg-app-bg px-4 py-5">
                {Picker && Preview && Panel1 && HueSlider && InputWidget && Swatches ? (
                  <Picker
                    value={draftValue}
                  ...
                  >
                    ...
                    <HueSlider style={{ borderRadius: 999, marginBottom: 16 }} />
                    <InputWidget
                      defaultFormat="HEX"
                      formats={['HEX']}
                      disableAlphaChannel
              ...
              <View className="mt-5 gap-3">
                ...
                />
                <Button title="Cancelar" variant="ghost" onPress={() => setVisible(false)} />
              </View>
            </View>
          </View>
        </Modal>
